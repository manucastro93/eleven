...
export default function SideCart(props: { abierto: boolean; onClose: () => void }) {
  ...
  return (
    <Show when={props.abierto}>
      ...
      {/* Panel lateral */}
      <div class="fixed top-0 right-0 w-[500px] max-w-full h-full bg-white z-[9999] shadow-lg flex flex-col">
        {/* Header */}
        <div class="flex justify-between items-center px-5 py-4 border-b">
          <h2 class="text-lg font-medium tracking-wide uppercase">
            {paso() === 1 ? "Carrito de compras" : "Tus datos"}
          </h2>
        ...
        <div class="flex-1 overflow-y-auto">
          ...
          <Show when={paso() === 2}>
            <PasoFormulario onVolver={() => setPaso(1)} />
          </Show>
        </div>
      </div>
    </Show>
