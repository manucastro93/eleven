...
export default function ModalClientesTop(props: {
  clientes: ClienteTop[];
  onClose: () => void;
}) {
  return (
      ...
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[85vh] overflow-y-auto">
          <div class="flex justify-between items-center p-4 border-b sticky top-0 bg-white z-10">
            <h2 class="text-xl font-semibold">Clientes que más compraron</h2>
                ...
                <For each={props.clientes}>
                  {(c) => (
                    <tr class="border-b last:border-none">
                      ...
                      <td class="py-2 px-3">{formatearPrecio(c.total)}</td>
                      <td class="py-2 px-3">{c.cantidadPedidos}</td>
                    </tr>
                  )}
                </For>
              </tbody>
